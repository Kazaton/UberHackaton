...
class User(AbstractUser):
    ...
    username = None
    USERNAME_FIELD = 'phone' # setting phone number as a username field when we login/register
    REQUIRED_FIELDS = []
    objects = UserManager()
...
class Bus(models.Model):
    ...
    # Number of people, seats and special seats (for disabled) in the bus currently
    number_of_people = models.PositiveIntegerField(default=0)
    number_of_seats = models.PositiveIntegerField(default=0)
    number_of_special_seats = models.PositiveIntegerField(default=0)
